...
class MicrophoneStream:
    ...
    def generator(self: object) -> object:
        ...
        """
        while not self.closed or not self._buff.empty():
...
def listen_print_loop(responses: object, silence_threshold: float = 10.0) -> str:
    ...
    for response in responses:
        ...
        if not result.is_final:
            ...
            #print(f"Time since last audio: {elapsed_time}")
            #if re.search(r"\b(exit|quit)\b", transcript, re.I):
            if time.time() - last_audio_time > silence_threshold:
                #print(f"Automatic stop due to {silence_threshold} seconds of silence.")
                print("Exiting..")
                break
...
def main() -> None:
    ...
    with MicrophoneStream(RATE, CHUNK) as stream:
        ...
        script = listen_print_loop(responses)
        promptspeech(script)
